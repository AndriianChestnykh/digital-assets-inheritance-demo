...
const initialize = async () => {
  ...
  async function getBalance () {
    try {
      ...
      accBalance.innerHTML = `${ethers.utils.formatEther(balance.toString())} ETH`
    } catch (err) {
      console.error(err)
    }
  }
  ...
  async function sendEther () {
    try {
      ...
      const transactionParameters = {
        ...
        value: ethers.utils.parseEther("0.1").toString(), // Only required to send ether to the recipient from the initiating external account.
        // data:
